...
function getApiKey(): string {
  ...
  if (!apiKey) {
    throw new Error(
      "Hypixel API Key not found in environment. Please notify administrator, and if you are the administrator, please get your shit together!",
    );
  }
  return apiKey;
}

async function requestHypixelData<T extends HypixelBaseResponse>(
  endpoint: string,
): Promise<T> {
...
export async function getBazaarPriceDataAvg(
  itemId: string,
): Promise<Map<string, number>> {
  const bazaarData =
    await requestHypixelData<HypixelBazaarResponse>("skyblock/bazaar");
...
export async function getItemNamesAndIDs(): Promise<HypixelItem[]> {
  const data = await requestHypixelData<HypixelItemsResponse>(
  ...
  if (!Array.isArray(itemsArray)) {
    throw new Error(
      "Invalid response format: 'items' array missing from Hypixel API.",
    );
  }
